...
export const ModalSaveEstimationAuthenticatedOutsideProjet = ({
  ...
  const validate = async () => {
    if (selectedProjetId > 0) {
      ...
      modal.close();
      router.push(PFMV_ROUTES.ESPACE_PROJET_LISTE_ESTIMATION(selectedProjetId));
    }
  };
  ...
  return (
    <modal.Component title="Sélection du projet" size="large">
      {isEmpty(projets) ? (
        <>
          <div className="mb-12">Vous devez créer un projet pour pouvoir estimer des solutions.</div>
          <Button
            priority="primary"
        ...
        <>
          <span>Dans quel projet souhaitez-vous estimer cette solution ?</span>
