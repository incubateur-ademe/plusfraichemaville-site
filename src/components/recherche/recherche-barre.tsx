...
export const RechercheBarre = ({ query, className }: RechercheBarreProps) => {
  ...
  const { data, error, isLoading } = useImmutableSwrWithFetcher<SearchResult>(FUZZY_SEARCH(query));
  if (error) {
    ...
    <>
      <SearchBar
        ...
        renderInput={({ className, id, placeholder, type }) => (
          <input className={className} defaultValue={query} placeholder={placeholder} type={type} id={id} autoFocus />
        )}
        defaultValue={query}
      />
      {error && <div>{"Une erreur s'est produite pendant la recherche, veuillez réessayer ultérieurement."}</div>}
      {isLoading && <RechercheResultatsSkeleton className="mt-10" />}
      {data && <RechercheResultats searchResult={data} className="mt-10" />}
    </>
  );
};
