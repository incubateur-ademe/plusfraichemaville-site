...
type ProjetVisibilityReducedProps = {
...
export const ProjetVisibilityReduced = ({
  isPublic,
  isLoading,
  onVisibilityChange,
  disabled,
}: ProjetVisibilityReducedProps) => {
  return (
    <div className="rounded-[20px] bg-dsfr-background-default-grey-hover px-4 pt-4">
      ...
      <div className="flex items-center justify-between">
        {!isLoading ? (
          <p
            className={clsx(
              "fr-badge fr-badge--sm fr-badge--no-icon mb-2 h-fit",
              isPublic ? "fr-badge--success" : "!text-dsfr-background-flat-red-marianne",
            )}
          >
            {isPublic ? (
              <Image
                src="/images/sourcing/sourcing-france-visible.svg"
                className="mr-1"
                width={11}
                height={11}
                alt=""
            ...
            {isPublic ? "Visible" : "Masqué"} {"sur l'annuaire"}
          </p>
        ) : (
