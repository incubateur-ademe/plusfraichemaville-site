...
export async function FichesSolutionsSante() {
  ...
  const filteredFichesSolution = fichesSolutions
    .filter((fs) => fs.cobenefices?.find((cb) => cb.icone === Icone.AmeliorerBienEtreSante))
    .sort((a, b) => {
      const indexA = TYPE_SOLUTION_ORDER.indexOf(a.type_solution as TypeSolution);
      const indexB = TYPE_SOLUTION_ORDER.indexOf(b.type_solution as TypeSolution);
  ...
  return (
    <ul className="m-0 mt-6 flex gap-6 overflow-x-auto pb-4 pl-0">
      {filteredFichesSolution.map((ficheSolution) => (
        <li key={ficheSolution.documentId} className="flex shrink-0 list-none">
