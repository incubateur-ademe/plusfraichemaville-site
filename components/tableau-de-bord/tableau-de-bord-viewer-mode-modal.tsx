...
export const TableauDeBordViewerModeModal = () => {
  ...
  const isLecteur = (currentProject && getCurrentUserRole(currentProject.users, userId) !== "ADMIN") ?? false;
  ...
  useEffect(() => {
    setTimeout(() => {
      if (!hasAlreadyDiscard && isLecteur) {
        modal.open();
      }
    }, 0);
  }, [hasAlreadyDiscard, isLecteur, modal]);
