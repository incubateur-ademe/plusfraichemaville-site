...
export const UserInfoForm = ({ user }: { user: User }) => {
  ...
  const form = useForm<UserInfoFormData>({
    ...
    defaultValues: { nom: user.nom ?? "", prenom: user.prenom ?? "", email: user.email, poste: user.poste ?? "" },
  });
  ...
  return (
    <form id="user-info" onSubmit={form.handleSubmit(onSubmit)}>
      ...
      <InputFormField control={form.control} path="email" label="Email" asterisk={true} />
