...
export default function RegionFilter({ className }: { className?: string }) {
  const { changeRegionFilterQueryString, clearRegionFilterQueryString, isRegionSelected } = useRegionFilter();
  ...
  return (
    <div className={`flex flex-row md:flex-col justify-center flex-wrap ${className}`}>
      <button className="fr-tag fr-text--xs mb-3" onClick={() => clearRegionFilterQueryString()}>
        Toutes régions
      </button>
      {ALL_REGIONS.map((region) => (
        <button
          key={region.code}
          onClick={() => changeRegionFilterQueryString(region.code)}
