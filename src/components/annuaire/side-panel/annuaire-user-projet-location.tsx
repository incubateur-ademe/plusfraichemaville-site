...
import { AnnuaireProjetVisibility } from "@/src/components/annuaire/annuaire-projet-visibility";
import { useProjetsStore } from "@/src/stores/projets/provider";
import { useIsLecteur } from "@/src/hooks/use-is-lecteur";

export const AnnuaireUserProjetLocation = () => {
  const currentProjet = useProjetsStore((state) => state.getCurrentProjet());
  const isLecteur = useIsLecteur(currentProjet?.id);
  return (
    <div className="flex h-full flex-col justify-between">
      <div className="m-auto flex flex-col items-center px-9">
        <Image src="/images/annuaire/annuaire-my-location.svg" width={80} height={80} alt="" className="mb-4" />
      ...
      <div className="px-4 pb-2">
        <AnnuaireProjetVisibility isLecteur={isLecteur} reduced />
      </div>
    </div>
  );
};
